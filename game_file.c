...
void initWorld(sfRenderWindow *window, sfEvent event){
    ...
    for(int i = 0; i < WIDTH/cellSize; i++){
        for(int j = 0; j < HEIGHT/cellSize; j++) {
            ...
            if (rand() % 4 == 0) sfRectangleShape_setFillColor(cell,sfBlack);
            else if (rand() % 4 == 1) sfRectangleShape_setFillColor(cell,colorPlanqton);
            else if (rand() % 4 == 2) sfRectangleShape_setFillColor(cell,colorFish1);
            else sfRectangleShape_setFillColor(cell,colorFish2);
