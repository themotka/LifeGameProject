        ...
        for (int j = 0; j < m; j++) {
            ...
            switch (type) {
                case Plankton: {
                    ...
                    //движение мха
                    if (!world[per(i - vertical, n)][per(j - horizontal, m)]->isAlive &&
                        currentCreature->lifeExpactancy > 1) {
                        currentCreature->lifeExpactancy--;
                        replaceCell(currentCreature, world[per(i - vertical, n)][per(j - horizontal, m)]);
                    }
                        //размножение
                    else if (!world[per(i - vertical, n)][per(j - horizontal, m)]->isAlive &&
                             currentCreature->lifeExpactancy == 1) {
                    ...
                    if (currentCreature->lifeExpactancy > 2) {
                        if (whatAboutHunter == -1) {//рядом нет хищников
                            if (whatAboutPlankton == -1) {//еды рядом нет, двигаемся в случайную сторону
                                ...
                                currentCreature->lifeExpactancy--;
                                currentCreature->lifeExpactancy = min(currentCreature->lifeExpactancy + 2,
                                                                      LIFE_EXPECTANCY_FISH);
                                replaceCell(currentCreature, world[newI][newJ]);
                                freeCell(currentCreature);
                            }
                        } else if (whatAboutHunter >
                                   -2) {//рядом есть хищники, убегаем (а лучше едим в это время планктона)
                        ...
                        newI = whatAboutHunter / m, newJ = whatAboutHunter % m;
                        if (whatAboutHunter >= 0 &&
                            world[newI][newJ]->type == Plankton) {//если есть хищники, и можно убегая поесть, just do it
                            currentCreature->lifeExpactancy--;
                            currentCreature->lifeExpactancy = min(currentCreature->lifeExpactancy + 2,
                                                                  LIFE_EXPECTANCY_FISH);
                            replaceCell(currentCreature, world[newI][newJ]);
                ...
                case Hunter: {
                    ...
                    if (whatAboutFish >= 0 && currentCreature->lifeExpactancy < 4) {
                        currentCreature->lifeExpactancy = min(LIFE_EXPECTANCY_HUNTERS,
                                                              currentCreature->lifeExpactancy + 2);
...
void printCnt (sfRenderWindow *window, creature ***world, int steps, sfFont *font){
...
void initWorld(sfRenderWindow *window, sfEvent event, creature ***world, int steps, sfFont *font) {
    ...
    printCnt(window, world, steps, font);
...
void gameCycle(sfRenderWindow *window, sfEvent event) {
    ...
    while (sfRenderWindow_isOpen(window)) { // тот же цикл отслеживания действий
        ...
        if (countFishes != 0 && countHunters != 0 && countPlankton != 0) {
            ...
            printf("Step: %d, Plankton: %d, Fish: %d, Hunters: %d\n", steps++, countPlankton, countFishes,
                   countHunters);
            usleep(20000);
            update(world, countPlankton, countFishes, countHunters);
        } else {
            sfRenderWindow_clear(window, sfWhite);
            sfRectangleShape *fillWorld = sfRectangleShape_create();
            sfVector2f size = {WIDTH, HEIGHT};
            sfRectangleShape_setSize(fillWorld, size);
            sfRectangleShape_setFillColor(fillWorld, sfWhite);
            printCnt(window, world, steps, font);
            sfTexture *texture = sfTexture_createFromFile("images/end.png", NULL);
            sfSprite *sprite = sfSprite_create();
            sfSprite_setTexture(sprite, texture, sfTrue);
            sfVector2f pos = {(WIDTH-684)/2 , (HEIGHT-388)/2-100};
